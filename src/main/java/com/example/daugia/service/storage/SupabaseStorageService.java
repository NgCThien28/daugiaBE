...
public class SupabaseStorageService {
    ...
    public UploadResult uploadKycImage(String matk, MultipartFile file, int maxMb) throws Exception {
        validate(file, maxMb);
        String safeName = sanitize(Objects.requireNonNull(file.getOriginalFilename()));
        String bucket = props.getBucketKyc();
        String objectPath = "kyc/" + matk + "/" + safeName;
        ...
        if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
            ...
            throw new RuntimeException("Upload Supabase KYC thất bại: " + resp.statusCode() + " - " + resp.body());
        }
    }
